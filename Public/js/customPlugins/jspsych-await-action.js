...
jsPsych.plugins["await-action"] = (function() {
...
/* -----------------------------Modification--------------------------------- */
    ...
    multiplier : {
      ...
      default: null,
      description: 'How much is the amount sent multiplied by'
    },
/* -------------------------------------------------------------------------- */
      prompt: {
        type: jsPsych.plugins.parameterType.STRING,
...
/* -----------------------------Modification--------------------------------- */
    ...
    socket.on('investor has chosen', function(action){
        //Stop the timeout
        ...
        let resultsHTML = `<div id='instructions-wrap'> <p> The investor has chosen to invest ${action} points. You have received ${trial.multiplier * action} points. </p>`;
        ...
        var investorAction = action;

        //Tell the server that this user received the results
        //socket.emit('trustee received results', action);
        ...
        $(".choice-btn").click(function(e){
            after_response(0, translucent_wait, action);
        });
    });
/* -------------------------------------------------------------------------- */
...
/* -----------------------------Modification--------------------------------- */
    ...
    function after_response(choice, translucent_wait,action) {
      ...
      if (trial.response_ends_trial) {
        end_trial(translucent_wait,action);
      }
    };

    // function to end trial when it is time
    function end_trial(translucent_wait, investorAction) {
      ...
      //Tell the server that this user received the results
      socket.emit('trustee received results', investorAction);
