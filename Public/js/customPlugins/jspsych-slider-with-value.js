...
jsPsych.plugins['slider-with-value'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    console.log("I'm being executed pre message");

    // Listen for message from server on investor taking action
    socket.on('send investor funds',function(data){
      console.log("I know investor took action");
      console.log(data);
      draw_html(data);
    });

    console.log("I'm being executed post message");

    function draw_html(dat){

     trial.max = trial.multiplier * dat
     trial.labels = ["0", trial.max.toString()]
   ...
   /* -----------------------------Modification--------------------------------- */
     ...
     function updateSlideValueOutput(){
        ...
        let outputValue1 = "Keep " + (trial.max  - jsPsychSlider.value);
        //The second information is x% of the last label
        let outputValue2 = "Send " + (jsPsychSlider.value);
